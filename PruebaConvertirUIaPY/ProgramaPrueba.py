...
class Principal(QtGui.QMainWindow):
 def __init__(self, parent=None):
  ...
  #Para programa 1
  self.ui.buttonSeleccionarExcel.clicked.connect(self.abrirExcel)
  self.ui.buttonCalcularCarretera.clicked.connect(self.calCarreteras)

  #Para programa 2  
  self.ui.doubleSpinBoxLC1.setVisible(False)
  self.ui.doubleSpinBoxCosC1.setVisible(False)
  self.ui.doubleSpinBoxTC1.setVisible(False)
  self.ui.doubleSpinBoxPC1.setVisible(False)
   
  
  self.ui.spinBoxCantCarreteras.valueChanged.connect(self.agregarCarreteras)#Se activa cuando se modifica el spinbox del programa 2
  self.agregarCarreteras()
  self.connect(self.ui.comboBoxL, QtCore.SIGNAL("currentIndexChanged(const QString&)"), self.cambiarSpinboxL)
  self.connect(self.ui.comboBoxCos, QtCore.SIGNAL("currentIndexChanged(const QString&)"), self.cambiarSpinboxCos)
  self.connect(self.ui.comboBoxT, QtCore.SIGNAL("currentIndexChanged(const QString&)"), self.cambiarSpinboxT)
  self.connect(self.ui.comboBoxP, QtCore.SIGNAL("currentIndexChanged(const QString&)"), self.cambiarSpinboxP)

  #Para programa 3
  self.ui.spinBoxOrden.valueChanged.connect(self.agregarCampos)#Para la tabla del programa 3
  self.ui.tableWidgetDatosEstructura.cellChanged.connect(self.validarEntrada)
  self.ui.pushButtonEvaluarEstructura.clicked.connect(self.evaluar)
 ...
 def agregarCarreteras(self):#Para el programa 2
     Programa2.generarInterfaz(self.ui.spinBoxCantCarreteras.value(), self.ui.tableWidgetRutas)

     
 def calCarreteras(self):#Para el programa 2 Recoger
     Programa2.analizarMatrices(self.ui.spinBoxCantCarreteras.value(), self.tableWidgetRutas,self.labelResultados, 
                                self.ui.comboBoxL.currentText() ,
                                self.ui.comboBoxCos.currentText(), self.ui.comboBoxT.currentText(), self.comboBoxP.currentText(),
                                self.ui.doubleSpinBoxLC1.value(), self.ui.doubleSpinBoxLC2.value(),
                                self.ui.doubleSpinBoxCosC1.value(), self.ui.doubleSpinBoxCosC2.value(),
                                self.ui.doubleSpinBoxTC1.value(), self.ui.doubleSpinBoxTC2.value(),
                                self.ui.doubleSpinBoxPC1.value(), self.ui.doubleSpinBoxPC2.value()
                                )
